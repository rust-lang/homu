...
def create_events(*args, into=None):
    if into is None:
        into = []

    for arg in args:
        into.append(create_event(arg))
...
@unittest.mock.patch('homu.pull_req_state.assert_authorized', side_effect=return_true)
def test_approved(_):
    """
    ...
    event = create_event({
        ...
        'body': 'Commit abcdef has been approved\n\n<!-- @bors r=ferris abcdef -->',
        'publishedAt': '1985-04-21T00:00:00Z',
    })
    result = state.process_event(event)
    assert result.changed is True
    ...
    result = state.process_event(event)
    assert result.changed is False
    assert assert_comment(r'`012345` is not a valid commit SHA', result.comments)
    assert state.get_status() == ''
...
#
#
#def test_approved_changed_push():
#    """
...
#
#
#def test_approved_changed_base():
#    """
